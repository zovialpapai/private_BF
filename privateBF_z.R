...
# BF for z test: Functions  ------------------------------------------------------------------
# Calculate Log Bayes Factor
ztest_BF <- function(z, tau, r = 1){
third  = (tau*z)^2/(2*(1 + tau^2))
ret = -(r + 0.5)*log(tau^2 + 1) + 
      log(hypergeom1F1(a = r + 0.5, b = 0.5 , z = third, n = 100)$f)

return(ret)
}

# BF for z test: Repeated Simulation in non-private set up  ------------------------------------------------------------------
### Test
n = 1000; mu = 0; 
alpha = 0.05
nrep = 10^4
BF_store = rep(NA, nrep)
# Data
for(m in 1:nrep){
x = rnorm(n, mu, 1)
set.seed(123 + m)
...
tau = 1
# Calculate
out = z.test(x,sigma.x=1)
BF_store[m] = ztest_BF(z = out$statistic, tau, r = 1)
}
hist(BF_store,  probability = T, 
     breaks = 20,
     xlab = "log(BF)", main = "Distribution of Bayes Factor (Under Null)")
cutoffs = quantile(BF_store, prob = c(alpha/2, 1 - (alpha/2)))
np_cutoffs = cutoffs
abline(v = cutoffs[1], col = "red", lwd = 2)
abline(v = cutoffs[2], col = "red", lwd = 2)

# BF for z test: Repeated Simulation in Private set up  ------------------------------------------------------------------
### Test
n = 1000; mu = 0;
alpha = 0.05
nrep = 10^4
D = 10 # Number of Partitions (Vary)
a = 3 # Partion-wise BF Cutoff (Vary, seq(1, 10, by = 1))
epsilon = 1 # Privacy Parameter (Vary, seq(0.5, 5, by = 0.5))
...
for(m in 1:nrep){
  ...
  out = rep(NA, D)
  for(k in 1:D){
    temp = z.test(x[((k - 1)*D + 1):(k*D)], sigma.x=1)
    out[k] = ztest_BF(z = temp$statistic, tau, r = 1)
    if(out[k] > a){out[k] = a}
    if(out[k] < -a){out[k] = -a}
  }
  set.seed(123 + m)
  eta = rlaplace(1, location = 0, scale = (2*a)/(sqrt(D)*epsilon))
  BF_store[m] = mean(out) + eta
}
hist(BF_store,  probability = T, 
     breaks = 20,
     xlab = "log(BF)", main = "Distribution of Private Bayes Factor (Under Null)")
cutoffs = quantile(BF_store, prob = c(alpha/2, 1 - (alpha/2)))
abline(v = cutoffs[1], col = "green", lwd = 2)
abline(v = cutoffs[2], col = "green", lwd = 2)
abline(v = np_cutoffs[1], col = "red", lwd = 2)
abline(v = np_cutoffs[2], col = "red", lwd = 2)
