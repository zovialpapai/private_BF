...
# BF for t test: Functions  ------------------------------------------------------------------
# Calculate Log Bayes Factor
t_2sided = function(r = 1, t, nu, tau2){
  tp1 = tau2 + 1
  a = tau2/tp1
  arg = t^2*a/(t^2+nu)
  bf = tp1^(-r-0.5)*Gauss2F1((nu+1)/2,r+0.5,0.5,arg) +
    sign(t)*sqrt(arg)*tp1^(-r-0.5)*
    exp(lgamma(nu/2+1)+lgamma(r+1)-lgamma((nu+1)/2)-
          lgamma(r+0.5))*
    Gauss2F1(0.5*nu+1,r+1,1.5,arg)
  return(log(bf))
}

# BF for t test: Repeated Simulation in non-private set up  ------------------------------------------------------------------
### Test
n = 1000; mu = 0; 
alpha = 0.05
nrep = 10^4
...
# BF for t test: Repeated Simulation in Private set up  ------------------------------------------------------------------
### Test
n = 1000; mu = 0;
alpha = 0.05
nrep = 10^4
